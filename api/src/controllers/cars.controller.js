...
// import orm.models.Car from '../models/cars.model'
...
controller.getAll = async (req, res) => {
  try {
    const cars = await orm.models.Car.getAll();
...
controller.addCar = async (req, res) => {
  let carToAdd = orm.models.Car({
    name: req.body.name
  });
  try {
    const savedCar = await orm.models.Car.addCar(carToAdd);
...
controller.deleteCar = async (req, res) => {
  ...
  try {
    const removedCar = await orm.models.Car.removeCar(carName);
